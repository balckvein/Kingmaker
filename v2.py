...
class Game:
    def __init__(self):
        ...
        self.resources = {}
        self.buildings = {
            "House": {"cost": {"Wood": 2, "Stone": 1}, "benefit": "Allows recruiting 1 villager"},
            "Lumber Hut": {"cost": {"Wood": 5, "Flint": 2}, "benefit": "Increases wood gathering", "upgrade_cost": {"Flint": 3}},
            "Quarry": {"cost": {"Stone": 6, "Flint": 3}, "benefit": "Increases stone gathering"},
            "Smithy": {"cost": {"Stone": 6, "Iron": 2}, "benefit": "Allows crafting tools and weapons"}
        }
    ...
    def build(self, building_type):
        if building_type in self.buildings:
            cost = self.buildings[building_type]["cost"]
            for resource, amount in cost.items():
                self.resources[resource] -= amount
            if "upgrade_cost" in self.buildings[building_type]:
                upgrade_cost = self.buildings[building_type]["upgrade_cost"]
                for resource, amount in upgrade_cost.items():
                    self.resources[resource] -= amount
            else:
                print("No upgrade cost")
            self.add_building(Building(building_type, str(cost), self.buildings[building_type]["benefit"], 
                                        upgrade_cost=self.buildings[building_type]["upgrade_cost"] if "upgrade_cost" in self.buildings[building_type] else None,
                                        upgrade_benefit=self.buildings[building_type]["benefit"] if "upgrade_cost" in self.buildings[building_type] else ""))
            print(f"Built a {building_type}")
        else:
    ...
    def play_turn(self):
        ...
        if choice == 1:
            ...
            building_type = list(self.buildings.keys())[choice - 2]
            self.build(building_type)
        elif len(self.buildings) + 2 <= choice <= (len(self.buildings) + len(self.available_characters)) * 2 + 1:
